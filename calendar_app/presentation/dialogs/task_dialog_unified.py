...
from PyQt6.QtWidgets import (
    ...
    QPushButton, QDateEdit, QMessageBox, QCheckBox, QWidget,
...
from calendar_app.infrastructure.google_sync.helpers import queue_task_delete_from_google, queue_task_sync_to_google
from calendar_app.presentation.dialogs.dialog_emoji import apply_dialog_title
from calendar_app.presentation.dialogs.dialog_styles import apply_common_dialog_style, polish_calendar_popup
...
class UnifiedTaskDialog(BaseTaskDialog):
    ...
    def __init__(self, parent=None, initial_date=None, initial_time=None, end_date=None,
                 task_type='schedule', template_id=None, end_time=None, task_id=None,
        ...
        if self._is_modify:
            ...
            title = t("dialog.task.reg_routine") if task_type == 'routine' else t("dialog.task.reg_schedule")
    ...
    def _build_basic_tab(self):
        ...
        name_label = t("dialog.task.name_routine") if self.task_type == 'routine' else t("dialog.task.name_schedule")
        name_hint = (
        ...
        schedule_group, schedule_layout = self._create_section(
            t("dialog.task.section_time_schedule") if self.task_type == 'schedule' else t("dialog.task.section_time_routine"),
            icon="⏰"
        )
        ...
        if self.task_type == 'schedule':
            ...
            e_time = self.preset_end_time if self.preset_end_time else self.initial_time.addSecs(3600)
            ...
            if not self._is_modify:
                ...
                QTimer.singleShot(0, lambda: self._on_all_day_toggled(self.all_day_check.isChecked()))
        ...
        if self.task_type == 'routine' and not self._is_modify and self.routine_period_end_date is not None:
    ...
    def _build_cycle_type_selector(self):
        ...
        self.repeat_mode_stack.setStyleSheet("QStackedWidget#RepeatModeStack { background: transparent; border: none; }")
        ...
        self.routine_wizard_placeholder.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed)
        ...
        self.routine_wizard_stack.setStyleSheet("QStackedWidget#RoutineWizardStack { background: transparent; border: none; }")
    ...
    def _refresh_checklist_display(self):
        ...
        for i in range(total):
            ...
            prefix = f"{i+1}. " if getattr(self, 'checklist_display_type', 'list') == 'process' else ""
        ...
        if hasattr(self, "progress_label"):
            self.progress_label.setText(f"{t('dialog.checklist.progress')}: {completed}/{total} ({percentage:.1f}%)")

    def _add_checklist_item(self):
        from PyQt6.QtWidgets import QInputDialog
        text, ok = QInputDialog.getText(self, t("dialog.checklist.add_item"), t("dialog.checklist.item_content"))
        if not ok or not text.strip():
            return
        if self._is_modify:
            order = self.checklist_widget.count()
            checklist_repo.add_checklist_item(self.task_id, text.strip(), order,
    ...
    def _remove_checklist_item(self):
        ...
        if self._is_modify:
            ...
            reply = QMessageBox.question(
                self, t("dialog.checklist.delete_title"), t("dialog.checklist.delete_confirm"),
        ...
        if self._is_modify:
            ...
            for item in items:
                checklist_repo.add_checklist_item(
                    self.task_id, item['item_text'], current_count,
                    display_type=display_type,
                )
                current_count += 1
            checklist_repo.set_task_checklist_display_type(self.task_id, display_type)
            self._load_checklist_items()
            QMessageBox.information(self, t("dialog.checklist.done"), f"{len(items)}{t('dialog.checklist.items_added')}")
        else:
            template = checklist_template_repo.get_checklist_template(template_id)
            ...
            for item in items:
                ...
                if item.get('item_description'):
                    tooltip_parts.append(f"{t('dialog.checklist.description', '설명')}: {item['item_description']}")
                if item.get('item_guide'):
                    tooltip_parts.append(f"{t('dialog.checklist.guide', '안내')}: {item['item_guide']}")
    ...
    def _load_from_template(self, template_id):
        ...
        self._apply_routine_recurrence(template.get('cycle_type') or 'single', template.get('recurrence'))
    ...
    def _load_template_checklist(self, template_id):
        ...
        if not row:
            return
        checklist_id = row['id']
        self.checklist_display_type = (row['checklist_type'] or 'list') if 'checklist_type' in row.keys() else 'list'
        cur.execute("""
    ...
    def _update_routine_preview(self):
        ...
        if count == 0:
            ...
            self.routine_preview_label.setText(f"📋 {count} {t('dialog.checklist.done')}  ({first} → {last})")
            self.routine_preview_label.setStyleSheet(
                f"color: {tokens.get('warning_hex', '#ffd34d')}; font-size: 14px;"
            )
        else:
            date_strs = [d.toString('M/d') for d in dates]
            self.routine_preview_label.setText(f"📋 {count} {t('dialog.checklist.done')}: {', '.join(date_strs)}")
            self.routine_preview_label.setStyleSheet(
    ...
    def _iter_routine_dates(self, start_date: QDate, end_date: QDate, cycle_type: str):
        ...
        def _next_date(d: QDate) -> QDate:
            if cycle_type == "daily":        return d.addDays(1)
            if cycle_type == "weekly":       return d.addDays(7)
            if cycle_type == "monthly":      return d.addMonths(1)
            if cycle_type == "quarterly":    return d.addMonths(3)
            if cycle_type == "half_yearly":  return d.addMonths(6)
            if cycle_type == "yearly":       return d.addYears(1)
            return d.addDays(1)
    ...
    def _copy_checklist_to_task(self, task_id: int):
        for i in range(self.checklist_widget.count()):
            ...
            link_id = checklist_repo.add_checklist_item(task_id, item_text, i, display_type=self.checklist_display_type)
    ...
    def _create_task(self):
        ...
        task_data = {
            ...
            'target_date': self.start_date.date().toString('yyyy-MM-dd') if self.task_type == 'routine' else None,
            'cycle_type': cycle_type,
            'all_day': 1 if hasattr(self, 'all_day_check') and self.all_day_check.isChecked() else 0,
            'description': self.memo_edit.toPlainText() if hasattr(self, 'memo_edit') and self.memo_edit else None,
            'location': self.location_edit.text().strip() if hasattr(self, 'location_edit') else None,
            'assignee': self.assignee_edit.text().strip() if hasattr(self, 'assignee_edit') else None,
        ...
        if is_repeat_routine:
            period_end = self.routine_period_end_date.date() if self.routine_period_end_date else self.start_date.date()
            if period_end < self.start_date.date():
                QMessageBox.warning(self, t("dialog.task.entry_error"), t("dialog.task.routine_end_error"))
                return
            ...
            for d in self._iter_routine_dates(self.start_date.date(), period_end, cycle_type):
                ...
                item_payload["deadline"] = f"{target_date} {self.start_time.time().toString('HH:mm:ss')}"
        ...
        if not created_ids:
            QMessageBox.critical(self, t("dialog.task.save_failed"), t("dialog.task.no_tasks_saved"))
            return
        ...
        if is_repeat_routine:
            ...
            success_msg = t("dialog.task.routine_reg_success") if self.task_type == 'routine' else t("dialog.task.schedule_reg_success")
    ...
    def _save_changes(self):
        ...
        updates = {
            ...
            'icon': None,
            'all_day': 1 if hasattr(self, 'all_day_check') and self.all_day_check.isChecked() else 0,
            'description': self.memo_edit.toPlainText().strip() if hasattr(self, 'memo_edit') and self.memo_edit else None,
            'location': self.location_edit.text().strip() if hasattr(self, 'location_edit') else None,
            'assignee': self.assignee_edit.text().strip() if hasattr(self, 'assignee_edit') else None,
            'calendar_id': self._get_selected_calendar_id(),
        }
    ...
    def _delete_task(self):
        ...
        reply = QMessageBox.question(
            self, t("dialog.task.del_title"), msg,
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
        )
        if reply == QMessageBox.StandardButton.Yes:
            if self.task_type == 'schedule':
            ...
            if success:
                ...
                QMessageBox.information(self, t("dialog.task.del_title"), t("dialog.task.del_success"))
                self.accept()
            else:
    ...
    def _build_calendar_selector(self, layout):
        ...
        default_index = 0
        try:
            from calendar_app.infrastructure.db.calendar_repo import list_calendars, get_default_calendar
