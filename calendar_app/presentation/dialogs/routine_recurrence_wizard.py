...
from calendar_app.presentation.dialogs.dialog_styles import build_dialog_stylesheet, get_dialog_theme_tokens
...
def _ordinal_name(number):
    if number == "last": return t("recurrence.last_week")
...
def normalize_recurrence_config(target_date, cycle_type, recurrence):
    ...
    if not parsed: return config
    config["mode"] = parsed.get("mode", config["mode"])
    if "slot" in parsed: config["slot"] = int(parsed["slot"])
    if "day" in parsed: config["day"] = int(parsed["day"]) if parsed["day"] != "last" else "last"
    if "nth" in parsed: config["nth"] = int(parsed["nth"]) if parsed["nth"] != "last" else "last"
    if "weekday" in parsed: config["weekday"] = int(parsed["weekday"])
    if cycle_type == "weekly": config["mode"] = "weekly"
    return config


def build_recurrence_rule(cycle_type, config):
    if cycle_type == "single": return "mode=single"
    if cycle_type == "weekly": return f"mode=weekly;weekday={config['weekday']}"
...
def cycle_slot_options(cycle_type):
    if cycle_type == "quarterly": 
        return [(t("recurrence.quarter_1"), 1), (t("recurrence.quarter_2"), 2), (t("recurrence.quarter_3"), 3)]
...
def recurrence_summary(target_date, cycle_type, recurrence):
    ...
    if config["mode"] == "nth_weekday":
        ...
        return t("recurrence.summary_nth", cycle=cycle_name, slot=slot_prefix, nth=nth, weekday=wd_names[config['weekday']])
    
    day_text = t("recurrence.last_day") if config["day"] == "last" else t("recurrence.suffix_day", n=config["day"])
...
def next_occurrence_text(target_date, cycle_type, recurrence):
    if cycle_type == "single": return t("recurrence.no_auto")
    next_date = routine_service.calculate_next_period(target_date.toString("yyyy-MM-dd"), cycle_type, recurrence)
...
class RoutineRecurrenceWizard(QDialog):
    def __init__(self, target_date, cycle_type="monthly", recurrence=None, parent=None):
        ...
        self.config = normalize_recurrence_config(self.target_date, self.selected_cycle_type, recurrence)
        ...
        tc = settings.value("theme_color", "#4da6ff")
        tc_hover = "#7abbff" # default fallback
        try:
            from PyQt6.QtGui import QColor
            c = QColor(tc)
            tc_hover = f"#{min(255, c.red()+20):02x}{min(255, c.green()+20):02x}{min(255, c.blue()+20):02x}"
        except: pass
            
        self.setStyleSheet(build_dialog_stylesheet() + _apply_wizard_theme_tokens(WIZARD_STYLE))
    ...
    def _build_ui(self):
        ...
        for value, label in get_cycle_labels().items():
            ...
            if value == self.selected_cycle_type: radio.setChecked(True)
        ...
        for day in range(1, 32): self.day_combo.addItem(t("recurrence.suffix_day", n=day), day)
        ...
        for text, value in [
            (t("recurrence.ordinal_1"), 1), (t("recurrence.ordinal_2"), 2), (t("recurrence.ordinal_3"), 3),
            (t("recurrence.ordinal_4"), 4), (t("recurrence.ordinal_5"), 5), (t("recurrence.last_week"), "last")
        ]:
        ...
        for idx, name in enumerate(get_weekday_names()): self.weekday_combo.addItem(name, idx)
        ...
        self.finish_btn.setProperty("class", "primary")
    ...
    def _apply_config_to_controls(self):
        for combo, key in [
            (self.rule_mode_combo, "mode"), (self.slot_combo, "slot"),
            (self.day_combo, "day"), (self.nth_combo, "nth"), (self.weekday_combo, "weekday")
        ]:
            val = self.config.get(key)
            if key == "mode": val = "nth_weekday" if val == "nth_weekday" else "day_of_month"
            idx = combo.findData(val)
            if idx >= 0: combo.setCurrentIndex(idx)

    def _sync_from_controls(self):
        btn = self.cycle_group.checkedButton()
        if btn: self.selected_cycle_type = btn.property("cycle_value")
        self._populate_slot_combo()

        self.config["mode"] = "weekly" if self.selected_cycle_type == "weekly" else self.rule_mode_combo.currentData()
        if self.slot_combo.count(): self.config["slot"] = self.slot_combo.currentData()
        ...
        self.lbl_slot.setVisible(has_slot); self.slot_combo.setVisible(has_slot)
        self.lbl_day.setVisible(not is_weekly and not is_nth); self.day_combo.setVisible(not is_weekly and not is_nth)
        self.lbl_nth.setVisible(not is_weekly and is_nth); self.nth_combo.setVisible(not is_weekly and is_nth)
        self.lbl_wkd.setVisible(is_weekly or is_nth); self.weekday_combo.setVisible(is_weekly or is_nth)

        recurrence = build_recurrence_rule(self.selected_cycle_type, self.config)
        self.summary_label.setText(recurrence_summary(self.target_date, self.selected_cycle_type, recurrence))
        self.next_label.setText(t("recurrence.next_occurrence_prefix", date=next_occurrence_text(self.target_date, self.selected_cycle_type, recurrence)))
    ...
    def get_result(self):
        ...
        return {
            ...
            "next_occurrence": next_occurrence_text(self.target_date, self.selected_cycle_type, recurrence),
        }
