...
일반 문구는 중괄호 없이 입력합니다. 중괄호 안에는 해당 위젯이 지원하는 변수나 아래 제어 토큰만 둘 수 있습니다.

```
{align=left} / {align=center} / {align=right}
{lh=1.4} / {line=150%} / {line_height=32pt}
```

프리셋 저장 시 `overlay_template_validation.py`가 다음을 검사합니다.

- 위젯 종류와 맞지 않는 변수
- 닫히지 않은 중괄호와 조건 블록, 지원하지 않는 중첩 조건
- `bold`, `italic`, `size=...`, `color=...`, 줄 간격 이외의 스타일 힌트
- 의미 색상 이름 또는 3·4·6·8자리 hex가 아닌 색상
- 지원 범위를 벗어난 정렬과 크기/줄 간격 값

앱 기본 프리셋은 같은 검사기를 통과해야 하며, JSON 카탈로그와 코드 fallback은 동일한 항목을 유지합니다.
