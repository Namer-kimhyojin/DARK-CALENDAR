(function () {
  ...
  const screens = {
    ko: {
      ...
      sync: { image: "assets/screenshots/calendar-sync.png", alt: "Air Calendar Google Calendar 동기화 설정 화면", kicker: "GOOGLE CALENDAR", title: "필요할 때만 연결하는 캘린더 동기화.", description: "연결, 캘린더 선택, 구독, 동기화, 진단을 단계별 탭으로 나누어 설정할 수 있습니다. 로컬 캘린더만으로도 사용할 수 있습니다.", points: ["선택형 Google Calendar 인증", "연결 테스트와 동기화 진단", "로컬·Google·ICS 캘린더 관리"] }
    },
    en: {
    ...
    ja: {
      ...
      sync: { image: "assets/screenshots/calendar-sync.png", alt: "Air Calendar Google Calendar同期設定", kicker: "GOOGLE CALENDAR", title: "必要なときだけつなぐカレンダー同期。", description: "接続、カレンダー選択、購読、同期、診断をタブごとに設定できます。ローカルカレンダーだけでも使用できます。", points: ["任意のGoogle Calendar認証", "接続テストと同期診断", "ローカル・Google・ICS管理"] }
    },
    zh: {
      ...
      sync: { image: "assets/screenshots/calendar-sync.png", alt: "Air Calendar Google Calendar 同步设置", kicker: "GOOGLE CALENDAR", title: "只在需要时连接日历同步。", description: "连接、日历选择、订阅、同步和诊断按步骤分成标签页。也可以仅使用本地日历。", points: ["可选 Google Calendar 授权", "连接测试与同步诊断", "本地、Google 与 ICS 日历管理"] }
    }
  };
